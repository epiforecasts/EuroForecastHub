...
#' @importFrom dplyr group_by mutate ungroup filter select bind_rows count
#'   summarise left_join select across n_distinct full_join distinct starts_with
...
summarise_scores <- function(scores, report_date, restrict_weeks = 0L) {
  ...
  ## extract data to be scored and set number of locations to one as default
  ## (see next command)
  score_data <- scores |>
  ...
  rel_ae <- score_df |>
    select(
      model, target_variable, horizon, location, location_name, forecast_date,
      ae_median
    ) |>
    pairwise_comparison(
      metric = "ae_median",
    ...
    select(
      model, target_variable, horizon, location, rel_ae = scaled_rel_skill
    ) |>
    distinct()
  ...
  table <- rel_ae |>
    full_join(
      rel_wis, by = c("model", "target_variable", "horizon", "location")
    ) |>
    full_join(
      msr_baseline, by = c("model", "target_variable", "horizon", "location")
    ) |>
    full_join(
      coverage, by = c("model", "target_variable", "horizon", "location")
    ) |>
    full_join(
      bias, by = c("model", "target_variable", "horizon", "location")
    ) |>
    left_join(
      num_fc, by = c("model", "target_variable", "horizon", "location")
    ) |>
    left_join(
      num_loc, by = c("model", "target_variable", "horizon", "location")
    ) |>
    left_join(
      locations, by = "location"
    ) |>
    mutate(across(any_of(c(
      "bias", "rel_wis", "msr_baseline", "rel_ae", "cov_50", "cov_95"
    )), round, 2))
